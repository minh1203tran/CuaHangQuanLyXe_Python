...
class UserApp:
    ...
    def khoa_mo_khoa_user(self):
        # 1. Lấy danh sách user từ DB
        users = self.controller.list_users()
        # if not users:
        #     return

        # 2. Hỏi người dùng nhập ID
        uid_str = self.get_input("Nhập ID user cần khóa/mở khóa (0 để hủy): ")
        if not uid_str or uid_str.strip() == "0":
            print("Hủy thao tác, quay lại menu chính!")
            return
        ...
        # 3. Kiểm tra ID có tồn tại trong danh sách không
        valid_ids = [u[0] for u in users]
        if uid not in valid_ids:
            print("ID không tồn tại trong danh sách!")
            return

        # 4. Hỏi người dùng muốn mở hay khóa
        choice = self.get_input("Nhập (m: mở, t: tắt, 0: hủy): ")
        ...
        self.controller.set_user_status(uid, status)
